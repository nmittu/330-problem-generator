...
module Model = struct
  ...
  let typ_t t = if Poly.(`Func = t.typ_config) then `Func t.func_count else `Norm
  ...
  let next t =
    ...
    { t with
      typ =
        Type_generator.generate ~mode:(mode t) ~typ:(typ_t t) t.params
        |> Type_generator.to_string
    ; code_input = ""
    ; compile_result = ""
    ; error = ""
  ...
  let compile t =
    ...
    | Ok compile_result ->
      ...
      then (
        ...
        then { t with compile_result = type_; error = "" }
        else
          { t with
            compile_result = ""
          ; error = Printf.sprintf "Incorrect type: %s" type_
          })
  ...
  let update_input t code_input =
    let model = { t with code_input; compile_result = ""; error = "" } in
    if String.is_suffix ~suffix:";;\n" code_input
    then compile { model with code_input = String.strip code_input }
    else model
  ;;
  ...
  and configs =
    ...
    let poly_on_click _ =
      Option.value
        ...
        (Option.map (get_int_val_by_id "poly_count") ~f:(fun n ->
           inject (Action.Update_mode (`Polymorphic n))))
    in
    let func_on_clic _ =
      Option.value
        ~default:(Ui_effect.return ())
        (Option.map (get_int_val_by_id "func_count") ~f:(fun n ->
           inject (Action.Update_typ (`Func n))))
    in
    let div_css =
      let ( @> ) = Css_gen.( @> ) in
    ...
    let mode_conf =
      Node.div
        ...
        ; Node.input
            ...
            []
        ; Node.label
            ~attr:(Attr.many_without_merge [ Attr.for_ "poly" ])
        ...
        ; Node.input
            ~attr:
              (Attr.many_without_merge
                 ...
                 ; Attr.on_input (fun _ i ->
                     inject (Action.Update_params (Int.of_string i)))
                 ; Attr.on_keydown (fun _ -> Effect.Prevent_default)
                 ])
            []
        ]
    in
    Node.div [ mode_conf; typ_conf; params_conf ]
  in
