...
let rec gen_func ~mode n =
  let ret =
    if n > 1 then
      gen_func ~mode (n-1)
    else gen_base ~mode () in
  Func (gen_base ~mode (), ret)

let generate_skeleton ~mode ~typ n =
  let get_fun_param_num () =
    max ((Random.int 5)/2) 1
  in
...
let generate ~mode ~typ n =
  ...
  | `Polymorphic a ->
     let alphabet = List.init a ~f:(fun i -> Char.of_int_exn (Char.to_int 'a' + i) |> Char.to_string) |> StringSet.of_list in
